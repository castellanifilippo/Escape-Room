...
//lstruttura server di gioco
struct Server{
    int players;    //numero players
    int state;      //partita avviata o meno
    int port;       //porta del server
    int running;    //se il server gira o meno
};

//struttura per richieste da parte del client
struct Request{
    int command;
...
void gameServer(struct Server *server, int id);
